...
import { unreadBoxStyle } from "./UserBoardStyling";
...
const UserBoard = () => {
  ...
  useEffect(() => {
    ...
    const userChannels = channelMessages.filter(
      (channel) => channel.members[userData?.username] && "id" in channel
    );
    const isUnread = userChannels.filter((channel) => {
      ...
      return channelMessages.some(
        (message) => message.timestamp > lastSeenUser
      );
    });
  ...
  useEffect(() => {
    ...
    const userDms = userDirectMessages.filter(
      (dm) => dm.members[userData?.username] && "id" in dm
    );
  ...
  return (
    <Stack direction="column" textAlign="center" gap={3}>
      <Typography variant="h3">User Board</Typography>
